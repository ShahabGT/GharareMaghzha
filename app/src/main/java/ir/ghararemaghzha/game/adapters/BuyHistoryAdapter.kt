...
class BuyHistoryAdapter(private val context: Context,
    ...
    override fun onBindViewHolder(h: ViewHolder, position: Int) {
        ...
        when (model.plan) {
            "1" -> h.title.text = context.getString(R.string.buyhistory_row_title, "500")
            "2" -> h.title.text = context.getString(R.string.buyhistory_row_title, "1000")
            "3" -> h.title.text = context.getString(R.string.buyhistory_row_title, "1500")
            "4" -> h.title.text = context.getString(R.string.buyhistory_row_title, "2000")
            "5" -> h.title.text = context.getString(R.string.buyhistory_row_title, "2500")
            else -> h.title.text = context.getString(R.string.buyhistory_row_title2)
        }
        h.price.text = context.getString(R.string.buyhistory_row_price, Utils.moneySeparator(model.amount))
        if (model.influencerCode != null && model.influencerCode.isNotEmpty()) {
            h.influencer.visibility = View.VISIBLE
            h.influencer.text = context.getString(R.string.buyhistory_influencer_price, Utils.moneySeparator(model.amount))
        } else h.influencer.visibility = View.GONE
        if (model.valid == "1") {
            h.stat.text = context.getString(R.string.buyhistory_row_stat, "موفق")
            h.resCode.visibility = View.VISIBLE
            h.resCode.text = context.getString(R.string.buyhistory_row_rescode, model.resCode)
        } else {
            h.stat.text = context.getString(R.string.buyhistory_row_stat, "ناموفق")
        ...
        h.date.text = context.getString(R.string.buyhistory_row_date, date.substring(11, 16) + " " + dateConverter.year + "/" + dateConverter.month + "/" + dateConverter.day)
